...
import type { ImageResult, Character, SeriesPrompt } from '../App';
...
const PreviewDisplay: React.FC<PreviewDisplayProps> = ({
    ...
    if (step === 1 && referenceImages.length > 0) {
        return (
             <div className="w-full h-full flex flex-col">
                <h3 className="text-lg font-semibold text-indigo-400 mb-4 text-center">Thư viện tham khảo phong cách</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 flex-grow overflow-y-auto pr-2">
                    {referenceImages.map(image => (
                        <div key={image.id} className="w-full aspect-square relative group bg-gray-900 rounded-lg shadow-md overflow-hidden">
                             <img
                                src={image.url!}
                                alt="Style Reference"
    ...
    if (step === 4 && (characters.some(c => c.preview) || isGeneratingChars)) { 
    ...
    if (step >= 6 && (generatedImages.length > 0 || isGeneratingSeries)) {
                ...
                <div className="w-full h-full flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                     ...
                     <ImageDisplay
                        ...
                        onProceedToStep8={onProceedToStep8}
    ...
    if (step === 8) {
        if (isGeneratingThumbnail && thumbnailResults.every(t => t.status === 'generating')) return <LoadingSpinner text="Đang tạo các phiên bản thumbnail..." />;
        if (thumbnailResults.length > 0) {
            return (
                 <div className="w-full h-full flex flex-col">
                    <h3 className="text-lg font-semibold text-indigo-400 mb-4 text-center">Kết quả Thumbnail (A/B Testing)</h3>
                    <p className="text-sm text-center text-gray-400 mb-4">Chọn thumbnail bạn ưng ý nhất. Mỗi thumbnail có thể được tải về hoặc chỉnh sửa riêng.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow overflow-y-auto pr-2">
    ...
    const hasAnyPreviews = contextPreview || characters.some(c => c.preview) || referenceImages.length > 0;
